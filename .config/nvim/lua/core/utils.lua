...
M.load_config = function()
    ...
    if chadrc_exists then
        ...
        if type(chadrc) == "table" then
            config.mappings = M.remove_disabled_keys(chadrc.mappings,
                                                     config.mappings)
...
M.load_mappings = function(section, mapping_opt)
    ...
    for _, sect in pairs(mappings) do set_section_map(sect) end
end
...
M.packer_sync = function(...)
    ...
    if git_exists and defaults_exists then
        ...
        if current_branch_name:match(defaults.snaps.base_snap_branch_name ..
                                         "(.+)" .. "$") then
            vim.api.nvim_echo({
...
M.closeAllBufs = function(action)
    ...
    for _, buf in ipairs(bufs) do M.close_buffer(buf) end
