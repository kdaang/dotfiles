...
function fzf_search_dir --description "Search the current directory. Replace the current token with the selected file paths."
    ...
    if string match --quiet -- "*/" $unescaped_exp_token && test -d "$unescaped_exp_token"
        ...
        set file_paths_selected $unescaped_exp_token($fd_cmd 2>/dev/null | fzf $fzf_arguments)
    else
        set --prepend fzf_arguments --prompt="Search Directory> " --query="$unescaped_exp_token" --preview='_fzf_preview_file {}'
        set file_paths_selected ($fd_cmd 2>/dev/null | fzf $fzf_arguments)
    end
